...
from  odoo.addons.api_collection.controllers.parser import Parser
from  odoo.addons.api_collection.controllers.exceptions import QueryFormatError
...
class Serializer(object):
    ...
    def build_flat_field(cls, rec, field_name):
        ...
        field_types = rec.fields_get(field_name).get(field_name)
        if field_types != None:
            ...
            if field_type in ['one2many', 'many2many']:
                ...
                if str(rec[field_name]).find('product.category') !=-1:
                    return {field_name: rec[field_name].complete_name}
                elif str(rec[field_name]).find('account.analytic.default') !=-1:
                    return {field_name: rec[field_name].analytic_id.name}
                elif str(rec[field_name]).find('stock.location') !=-1:
                    if str(rec[field_name]).find('stock.location.route') != -1:
                        return {field_name: rec[field_name].name}
                    else:
                        return {field_name: rec[field_name].complete_name}
                elif str(rec[field_name]).find('stock.picking.type') !=-1:
                    namawh = ''
                    if  rec[field_name].warehouse_id.name:
                        namawh = rec[field_name].warehouse_id.name + ":" + rec[field_name].name
                    else:
                        namawh = rec[field_name].name
                    return {field_name: namawh}
                else:
                    return {field_name: rec[field_name].name}
            elif field_type == 'datetime' and rec[field_name]:
                return {
                    field_name: rec[field_name].strftime("%Y-%m-%d %H:%M:%S")
                }
